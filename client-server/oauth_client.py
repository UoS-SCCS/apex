...
def config_oauth_client(app):
    oauth.init_app(app)
    oauth.register(name='mydrive', fetch_token=fetch_mydrive_token)
...
def authorize():
    ...
    newtoken = OAuth2Token(user_id=current_user.get_id(),token_type=token["token_type"],access_token=token["access_token"],expires_at=token["expires_at"],scope=token["scope"],oauth_uid=profile["user_id"])
